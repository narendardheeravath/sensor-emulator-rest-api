...
1.  Make sure that MongoDb is installed on machine before executing project
     a. Download the mongodb form https://www.mongodb.com/download-center#community
     b. Install the mongodb once download is complete
     c. Run mongod.exe from C:\Program Files\MongoDB\Server\3.2\bin through commond prompt
2. Run the /sensor-emulator-rest-api/src/main/java/io/egen/sensor/emulator/SensorApplication.java as SpringBootApplication
     a. Make sure that we have to pass VM Argument while running the SensorApplication.java and below is VM arguments
...
3. SpringBootApplication SensorApplication deployed into localhost and all rest end points are available to consume by Emulator once completes
     a. Below are the rest end points which exposed the below Metric APIs using Spring MVC,
	...
	b. Exposed the below Alert APIs using Spring MVC
...
4. Run the Emulator.java which I copied in same project as Java Application
     a. a. Make sure that we have to pass VM Argument while running the SensorApplication.java and below is VM arguments
        -Dbase.value=150 -Dapi.url=http://localhost:8080
...
Below is sample output from rest end point
...
Use MongoDB as your datastore with two collections
